...
func TestManagedFrontendFailureVisibleDirectFallback(t *testing.T) {
	for _, cause := range []string{"backend exited before publishing listener", "readiness timeout", "version mismatch", "another managed OpenCode frontend is already active"} {
		t.Run(cause, func(t *testing.T) {
			...
			if strings.Contains(cause, "already active") {
				if err == nil {
					t.Fatal("lease bypassed")
				}
				return
			}
			if err != nil {
			...
			if !strings.Contains(diagnostic.String(), "AUTO_STATE=DEGRADED") || !strings.Contains(diagnostic.String(), "native TUI") {
				t.Fatal("semantic switch not visible")
			}
			values, err := a.SessionList()
			if err != nil || len(values) != 1 || values[0].Frontend == "opencode" {
				t.Fatal("direct session mislabeled as OpenCode")
			}
		})
	}
}
