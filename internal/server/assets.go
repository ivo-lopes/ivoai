...
const ComposeYAML = `services:
  qdrant:
    ...
    volumes:
      ...
      - /var/lib/ivoai/qdrant-snapshots:/qdrant/snapshots
      - /var/lib/ivoai/qdrant-init:/qdrant/init
    healthcheck:
  ...
  embeddings:
    ...
    healthcheck:
      ...
      timeout: 3s
      retries: 30
      # The first CPU-only model download can exceed five minutes on a clean
      # host. Successful probes still end this grace period immediately.
      start_period: 10m
    security_opt: ["no-new-privileges:true"]
    networks:
      ivoai-internal: {}
      host-publish: {}
      # Force the only masqueraded network to be the startup default route.
      # systemd disconnects it immediately after the model is healthy.
      model-download:
        gw_priority: 1
...
ReadWritePaths=/var/lib/ivoai/enrollment /var/lib/ivoai/web-oauth /run/ivoai
...
InaccessiblePaths=/etc/ivoai/secrets /var/lib/ivoai/enrollment /var/lib/ivoai/web-oauth /var/lib/ivoai/memory
EnvironmentFile=/etc/ivoai/secrets/qdrant.env
EnvironmentFile=/etc/ivoai/secrets/embeddings.env
UMask=0077
...
const DocsUnit = `[Unit]
Description=IVOAI product documentation
...
User=ivoai-docs
Group=ivoai
ExecStart=/usr/local/bin/ivoai server docs serve
Restart=on-failure
RestartSec=5s
NoNewPrivileges=yes
PrivateTmp=yes
...
ReadOnlyPaths=/etc/ivoai
InaccessiblePaths=/etc/ivoai/secrets /var/lib/ivoai
UMask=0077
