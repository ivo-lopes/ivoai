...
The client-side knowledge path is provider-neutral. Unfiltered sessions select every
enabled server; repeated `--knowledge-source` flags form a restrictive session-local
subset:
...
- `CodexExecutor` and `ClaudeExecutor` over their official CLI runtimes, plus a
  standalone `OpenCodeExecutor`. AUTO uses OpenCode as a managed frontend and routes
  execution through the Codex/Claude contracts; the standalone OpenCode executor is
  still direct-only. They expose session start and bounded cancellation.
...
control plane. A pinned OpenCode TUI is the interactive frontend; it is not a second
scheduler and never becomes the authoritative writer. The TUI attaches to an
authenticated OpenCode server on a random `127.0.0.1` port. A private IVOAI provider
bridge on another random loopback port invokes the selected official Codex or Claude
CLI, so each client retains its own subscription authentication. No vendor token,
cookie, refresh token, or credential file is read or copied into OpenCode.

The OpenCode server plugin attaches non-secret frontend session and message IDs to
bridge requests. IVOAI persists a bounded, working-directory and knowledge-scope
bound mapping from that ID to separate official CLI conversation IDs per executor.
This supports subsequent turns, failover and restart recovery without storing
conversation bodies or crossing knowledge scopes. IVOAI remains planner, logical session owner, executor selector,
quota authority, policy authority and result consolidator. Its first substantive turn
attempts Memory then Context exactly once, creates a private bounded
SharedContextBrief, validates a maximum-12-task DAG, and submits numerical task
signals to IVOAI. IVOAI—not OpenCode—calculates capability scores, chooses the lowest
sufficient runtime-verified model/effort profile, rejects uneconomic delegation, and
runs dependency-ready advisory workers asynchronously.
...
hard limit it reaps the current executor process group, re-probes the alternate, and
continues inside the same OpenCode frontend with a bounded handoff. A two-failover
ceiling prevents ping-pong loops. The managed overlay disables OpenCode sharing,
auto-update and project configuration; running OpenCode outside IVOAI is unaffected. See
...
reads or stores the returned credential. Source: [ChatGPT authentication](https://learn.chatgpt.com/docs/auth).
...
Sources: [Headroom CLI](https://headroomlabs-ai.github.io/headroom/cli/),
[Headroom v0.36.0](https://github.com/headroomlabs-ai/headroom/releases/tag/v0.36.0),
[Headroom issue 940](https://github.com/headroomlabs-ai/headroom/issues/940),
[uv 0.12.5](https://github.com/astral-sh/uv/releases/tag/0.12.5), and
...
[ai-memory v1.29.0](https://github.com/akitaonrails/ai-memory/releases/tag/v1.29.0); its changelog was
...
[TEI v1.9.3](https://github.com/huggingface/text-embeddings-inference/releases/tag/v1.9.3) and
...
[multilingual-e5-small pinned revision](https://huggingface.co/intfloat/multilingual-e5-small/tree/614241f622f53c4eeff9890bdc4f31cfecc418b3).
