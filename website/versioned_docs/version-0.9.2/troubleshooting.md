...
IVOAI executor failure. The short error now includes a stable classification such as
`executor_exit_nonzero`, `executor_auth_failure`, `executor_stream_incomplete`,
`executor_timeout`, or `executor_cancelled`; partial output is never accepted as a
successful answer.

Run `ivoai doctor`, inspect the non-sensitive session state with `ivoai session show
--json <session-id>`, and verify the official client directly. Do not paste
credentials or raw tool results into a bug report. A federated Memory/Context call
must return an MCP `CallToolResult`; older v0.9.0 builds instead returned the
federation envelope directly and Codex reported `Unexpected response type`. Update
to the corrective release, then retry the original turn. If the classification is
an authentication failure, authenticate through the named official client rather
than OpenCode `/connect`.

## The model or reasoning option is missing

The managed catalog is discovered when AUTO starts. Open the native OpenCode model
picker and select an explicit IVOAI Codex/Claude entry; only verified reasoning
variants appear. `IVOAI Automatic Orchestration` intentionally has no fixed
reasoning variant because the scheduler owns that decision. If only the automatic
entry exists, run `ivoai doctor` and verify that the official clients are installed
and support capability discovery. Never enable a parallel provider login to make a
model appear.

## Memory or Context reports `Unexpected response type`

This indicates that a tools/call response crossed the MCP boundary with an invalid
result shape. In multi-server mode, IVOAI returns a canonical `CallToolResult` whose
text content contains the deterministic federated envelope and source provenance.
Run the Knowledge Router conformance test and compare a single-source call with the
same two-source call. Do not disable federation or remove a source merely to hide
the error. Corrective releases preserve exact single-source responses and validate
the federated response with the official Go MCP SDK.
...
transactional updater. Do not point AUTO at an unrelated global OpenCode binary.

## OpenCode reports a version or plugin mismatch

**Symptom:** backend health succeeds but the managed plugin does not load. Doctor
reports the expected and observed OpenCode versions without printing paths to
credentials. Run `ivoai update --dry-run`, then `ivoai update` if the reviewed
candidate is compatible. The plugin and OpenCode pin are one rollback unit; do not
install a floating plugin package or enable OpenCode auto-update in managed mode.

## Codex or Claude appears unauthenticated in the IVOAI panel

**Symptom:** `/ivoai` says `authentication required`, although a previous session was
valid. IVOAI never copies credentials into OpenCode. Verify the relevant official
client (`codex login status` or the normal Claude Code login surface), then use
`ivoai connect chatgpt` or `ivoai connect claude` to run the official flow and
invalidate the stale quota cache. Do not enter a provider key in OpenCode `/connect`;
managed AUTO has only the IVOAI provider enabled.
...
**Symptom:** the footer says `0 configured`, a selected source is down, or the panel
has not reflected a recent operation. Confirm profiles with `ivoai connect server
list` and use `ivoai connect server test <alias>`. The panel refreshes bounded status
every five seconds and incorporates observed selected-source operations. Automatic
mode keeps healthy selected sources and marks the session degraded; an explicitly
selected unavailable source fails without substitution.

## Theme, color, or narrow-terminal rendering is unavailable

The managed overlay selects the IVOAI theme through an official OpenCode plugin.
`NO_COLOR`, `TERM=dumb`, non-TTY and JSON workflows remain plain and do not receive
branding. Resize to at least a usable terminal size; long source labels are bounded
and the panel limits visible rows. If the terminal cannot render Unicode marks, use
`IVOAI_ASCII=1` on IVOAI's native status/menu surfaces.

## Managed OpenCode configuration is corrupted

Managed files live under private IVOAI runtime/state roots and are regenerated for a
session. Stop the session and rerun `ivoai setup` or a validated update. Do not copy
the user's global OpenCode config or credential store into the managed root. Project
`.opencode` configuration, sharing and auto-update are disabled only in managed mode;
direct `opencode` use remains upstream-owned.
...
On Debian 12, rerun `sudo ivoai setup --mode server`; IVOAI can install a missing
Engine from Docker's official signed repository after key verification. It will not
replace an unknown repository configuration or non-official old Engine. Otherwise,
...
## ChatGPT or Claude Web cannot connect to `/mcp` {#chatgpt-or-claude-web-cannot-connect-to-mcp}
...
Without a source flag, IVOAI automatically reads from every enabled connected
profile. Use `ivoai codex --knowledge-source mindsite` to restrict a session, and
repeat the flag for an exact subset. An ambiguous Memory write is rejected; restart
the operation with exactly one destination rather than disconnecting another source.
...
In LXC, `LXC_DETECTED=true` with an unreachable daemon usually means nesting,
`keyctl`, cgroup, or device permissions are missing at the host. Change those in the
Proxmox/LXC configuration, then rerun setup. `ivoai server doctor`, `memory status`,
and `context status` report `SERVER_SETUP=INCOMPLETE` with this prerequisite as the
root cause; missing generated `.env` files are only cascade symptoms.

### Documentation service is unavailable

Run `sudo ivoai server docs status`, `systemctl status ivoai-docs.service`, and
`journalctl -u ivoai-docs.service`. The service must listen on `0.0.0.0:7780` unless
`/etc/ivoai/docs.json` intentionally changes the port. Verify loopback first with
`curl -f http://127.0.0.1:7780/`, then the server LAN address. If loopback succeeds
but LAN fails, inspect the host firewall and the Nginx Proxy Manager source route;
do not replace the production service with `docusaurus start`.
