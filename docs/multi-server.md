...
Repeat the flag, or use a comma-separated value, only for intentional read
federation:
...
With one enabled server, or a migrated legacy `default` profile, no new source
flag is required. With multiple purposes and no `default`, omission is ambiguous
and fails rather than guessing an organization.
...
The router preserves authoritative single-source MCP responses. Explicitly
federated results add the source envelope needed for attribution. WorkingContext,
